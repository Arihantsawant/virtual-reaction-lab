// Centralized simulator config: types, constants, and helpers
...
export const SOLVENT_SMILES: Record<string, string> = {
  // Explicit hydrogens for water so the viewer renders H–O–H
  water: "[H]O[H]",
  ethanol: "CCO",
  methanol: "CO",
  isopropanol: "CC(O)C",
...
// Estimate time helper (display-only heuristic)
...
// Deterministic pseudo-score helper for mock metrics
export const scoreFromSeed = (seed: string, salt: string) => {
  let h = 2166136261 >>> 0;
  const s = seed + "|" + salt;
